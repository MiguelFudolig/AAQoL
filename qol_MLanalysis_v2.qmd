...
imbal_index <- createDataPartition(rfdata$Family,p=0.75,list=F,times=1)
...
<!-- mod1 <- glm(Family~.,data=train,family=binomial) -->
...
mod1w <- glm(Family~.,data=train,family=binomial) 
...
<!-- ### Without Ethnicity -->
<!-- ```{r} -->
<!-- x <- model.matrix(Family~.,data=train) -->
...
x_test <- model.matrix(Family~.,data=test)
...
```{r}
glm(Family~.,data=rfdata,family=binomial) -> mod1
glm(Family~.,data=rfdata |> select(-Ethnicity),family=binomial) ->mod1w
```
...
```{r}
mod<- train(Family~.,data=train,method="rf")
VarImp <- varImp(mod,scale=F)

print(VarImp)
plot(VarImp)

savePlot(filename="Variable Importance Plot.png",type="png")
```
...
```{r}
glm(Heal.Professionals~.,data=rfdata |> select(-Ethnicity),family=binomial) -> mod1
glm(Heal.Professionals~.,data=rfdata,family=binomial) ->mod1w
```
